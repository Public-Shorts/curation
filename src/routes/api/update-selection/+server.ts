...
Return ONLY a JSON object: { "summary": "your 2-3 paragraph text here" }`;
	...
	try {
		...
		return 'Summary generation failed. Ensure Ollama is running locally with the gemma3:4b model.';
	}
}
...
export const POST: RequestHandler = async () => {
	...
	try {
		...
		// 4. Generate summary
		const totalRuntime = [...highlights, ...selected].reduce(
		...
		const allScores = submissions
			...
			.filter((s: number) => s > 0);
		const avgScore =
			allScores.length > 0 ? allScores.reduce((a: number, b: number) => a + b, 0) / allScores.length : 0;
		...
		const outputPath = path.resolve(process.cwd(), 'src/lib/data/selection.json');
		fs.writeFileSync(outputPath, JSON.stringify(outputData, null, 2));

		return json({
			success: true,
			message: `Selection updated: ${allSelectedFilms.size} films (${highlights.length} highlights, ${selected.length} by score, ${vetoedIds.size} vetoed)`
		});
	} catch (error: any) {
		console.error('Error updating selection:', error);
		return json({ error: error.message || 'Failed to update selection data' }, { status: 500 });
	}
};
