<script lang="ts">
	...
	let allFilms = $derived([
		...(cluster.highlightedFilms || []),
		...(cluster.relevantFilms || []),
	]);
		...
		<div class="flex items-center justify-between w-full">
			...
			<div class="flex items-center gap-6 text-sm text-gallery-500 ml-4">
				<div class="text-right">
					<span class="font-semibold text-gallery-900">{cluster.filmCount}</span>
					<span class="ml-1">film{cluster.filmCount !== 1 ? 's' : ''}</span>
				</div>
