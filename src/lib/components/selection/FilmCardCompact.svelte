<script lang="ts">
	...
	let { film, onclick, onRemove }: Props = $props();
</script>
...
<div class="relative group w-full">
	...
	>
		...
		<div class="h-12 w-20 rounded bg-gallery-100 flex-shrink-0 overflow-hidden">
			...
			{:else if film.screenshots?.[0]?.asset}
				...
				/>
			{:else}
				<div class="h-full w-full flex items-center justify-center text-xs text-gallery-400">
					🎬
				</div>
			{/if}
		</div>
	...
	{#if onRemove}
		<button
			...
			class="absolute top-2 right-2 p-1 bg-red-500 text-white rounded-full opacity-0 group-hover:opacity-100 hover:bg-red-600 transition-all z-10 shadow-sm"
			title="Remove from screening"
		>
			<X class="w-3 h-3" />
		</button>
	{/if}
